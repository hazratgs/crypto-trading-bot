...
class Bitstamp extends Base {
  constructor(option) {
    ...
    this.socket = new Pusher('de504dc5763aeef9ff52', { cluster: 'mt1' })
    this.channel = this.socket.subscribe(`live_trades`)
  }
  ...
  trades() {
    this.channel.bind('trade', async item => {
        const type = item.type === 0 ? 'buy' : 'sell'
        await this.addElementCandles([item.type === 0 ? 'buy' : 'sell', item.price, item.amount])
    })
  }
